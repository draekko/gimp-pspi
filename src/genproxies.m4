...
dnl PROXY(type,suite,field,proc,arglist,fmtlist,fmtvals,argnames,resultfmt,outfmtlist,outvals)
...
ifelse($1,[OSErr],[  gchar *e;
  ...
  fprintf (log, "ifelse([$2],,,[$2::])$4 ($6)"ifelse([$6],,,[, ifelse([$7],,[$8],[$7])]));
  ifelse([$1],[void],,[result = ])orig_filter[]ifelse([$3],,,[->$3])->$4 ([$8]);
ifelse([$1],[void],,[$1],[OSErr],[  e = error_string (result);
  fprintf (log, ": %s", e);
],[ifelse([$9],,,[  fprintf (log, ": $9", result);
])])dnl
ifelse([$10],,,[  fprintf (log, ": $10\n", $11);
])dnl
  fprintf (log, "\n");
...
PROXY(OSErr, PropertyProcs, propertyProcs, getPropertyProc,
	...
	[%s, %s, %d], [int32_as_be_4c (signature), int32_as_be_4c (key), index],
...
PROXY(OSErr, , , colorServices,
	...
	[%s], [color_services_string (info->selector)], [info])
...
PROXY(OSErr, , , displayPixels,
	...
	[{%s}, %s, %d, %d, %p], [pspixelmap_string (source),
				 vrect_string (srcRect),
				 dstRow, dstCol, platformContext],
